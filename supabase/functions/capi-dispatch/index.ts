...
function passesFilters(ev: Record<string, unknown>, filters: Filters): boolean {
  ...
  const campaign = ((deal.utmCampaign ?? payload.utmCampaign) as string ?? '').trim()
  const source   = ((deal.utmSource   ?? payload.utmSource)   as string ?? '').toLowerCase().trim()
  const segment  = ((deal.segment     ?? payload.segment)     as string ?? '').trim()
  const revenue  = ((deal.revenue     ?? payload.revenue)     as string ?? '').trim()
  ...
  if (ev.event_type === 'deal_won') {
    const rawMrr = deal.potentialNewMRR as number | string | null | undefined
...
async function sendBatch(
  ...
  try {
    ...
    if (!res.ok) return { sent: 0, error: json?.error?.message ?? `HTTP ${res.status}` }
    return { sent: json.events_received ?? 0 }
  } catch (e) {
...
Deno.serve(async (req: Request) => {
  ...
  const body = await req.json().catch(() => ({})) as {
    auto?: boolean
    pixelId?: string      // capi_pixels.id (UUID) — dispatch only this pixel
    dateFrom?: string
    dateTo?: string
    enrichData?: Record<string, EnrichEntry>  // email_norm → enrichment
  }

  const sb = createClient(SUPABASE_URL, SERVICE_KEY)

  // ── Select which pixels to dispatch ────────────────────────────────────────

  let query = sb.from('capi_pixels').select('*').eq('enabled', true)

  if (body.pixelId) {
    query = query.eq('id', body.pixelId)
  } else {
    // Auto mode: only pixels with auto_dispatch = true
    query = query.eq('auto_dispatch', true)
  }

  const { data: pixels, error: pixelErr } = await query
  if (pixelErr) {
    return Response.json({ ok: false, error: pixelErr.message }, { headers: CORS, status: 500 })
  }
  ...
  for (const pixel of (pixels ?? [])) {
    ...
    if (!body.pixelId && body.auto) {
      if (pixel.last_dispatched_at) {
    ...
    // ── Active event types (non-empty mapping) ────────────────────────────
    ...
    const sentSet = new Set((sentRows ?? []).map((r: { event_id: string }) => r.event_id))
    ...
    for (const ev of toSend) {
      ...
      const rawMrr   = deal.potentialNewMRR as number | string | null | undefined
      const mrr      = (ev.event_type === 'deal_won' && rawMrr !== null && rawMrr !== undefined)
    ...
    summary.push({
      pixel: pixel.name,
      attempted: capiEvents.length,
      sent: totalSent,
      errors,
    })
  }

  return Response.json({ ok: true, dispatched: summary }, { headers: CORS })
})
