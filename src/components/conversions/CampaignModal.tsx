...
function QCard({
  label, value, delta, inverted = false,
}: { label: string; value: string; delta: number | null; inverted?: boolean }) {
  return (
...
function CeaFlowchart({ campaign, config, status }: { campaign: CampaignMetrics; config: CeaConfig; status: CeaStatus }) {
  const isEscala = status.stage === 'ESCALA'

  if (!isEscala) {
    const hasMqls = campaign.mqls > 0
    ...
    return (
      <div className="flex flex-col gap-2.5">
        ...
        <FlowStep
          check="MQLs"
          label={hasMqls ? `${campaign.mqls} MQLs gerados` : 'Sem MQLs'}
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <FlowStep check="Fase" label={`ESCALA (${campaign.meetings} reuniões)`} result="" ok={null} />
      <FlowStep
        check="Vendas"
        label={hasWins ? `${campaign.won} venda(s)` : 'Sem vendas ainda'}
...
export default function CampaignModal({
  ...
  const status = useMemo(() => computeCEAStatus(campaign, config), [campaign, config])

  const campaignFilters = useMemo(
    () => ({ channels, campaigns: [campaign.campaign], onlyActive }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [JSON.stringify({ channels, campaign: campaign.campaign, onlyActive })],
  )

  const qualityMetrics = useMemo(
    () => computeQualityMetrics(applyWindsorFilters(rawWindsorRows, campaignFilters)),
    [rawWindsorRows, campaignFilters],
  )
  ...
  useEffect(() => {
    ...
    fetchWindsorData(prevFrom, prevTo)
      .then((rows) => {
        if (cancelled) return
        setPrevMetrics(computeQualityMetrics(applyWindsorFilters(rows, campaignFilters)))
      })
      .catch(() => { /* silent */ })
    return () => { cancelled = true }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dateFrom, dateTo, JSON.stringify(campaignFilters)])
          ...
          <div className="px-6 py-4 border-b border-gray-100 flex items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              <h2 className="text-sm font-bold text-gray-800 leading-snug break-words">
                {campaign.campaignFullName ?? campaign.campaign}
              </h2>
              {campaign.campaignFullName && (
                <p className="text-[11px] text-gray-400 font-mono mt-0.5">{campaign.campaign}</p>
              )}
                ...
                {campaign.status && (
                  <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full whitespace-nowrap ${
                    ...
                    <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                      campaign.status === 'ENABLED' || campaign.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-gray-400'
                    }`} />
                    {campaign.status === 'ENABLED' || campaign.status === 'ACTIVE' ? 'Ativo' : 'Pausado'}
                  </span>
                )}
            ...
            {status && (
              <section>
                ...
                <div className="bg-gray-50 rounded-xl border border-gray-100 px-4 py-4">
                  <CeaFlowchart campaign={campaign} config={config} status={status} />
                </div>
              </section>
            )}
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    <tr>
                      ...
                      <td className="py-2 px-3 text-right font-medium text-gray-800">{fmtBRL(campaign.spend)}</td>
                      <td className="py-2 px-3 text-right text-gray-400">—</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-3 text-gray-600">MQLs</td>
                      <td className="py-2 px-3 text-right font-medium text-gray-800">{fmtN(campaign.mqls)}</td>
                      <td className="py-2 px-3 text-right text-gray-500">{ratio(campaign.spend, campaign.mqls)}</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-3 text-gray-600">SQLs</td>
                      <td className="py-2 px-3 text-right font-medium text-gray-800">{fmtN(campaign.sqls)}</td>
                      <td className="py-2 px-3 text-right text-gray-500">{ratio(campaign.spend, campaign.sqls)}</td>
                    </tr>
                    <tr className={campaign.sqls > 0 ? '' : 'opacity-50'}>
                      <td className="py-2 px-3 text-gray-600">MQL→SQL</td>
                      <td className="py-2 px-3 text-right font-medium text-gray-800">
                        {campaign.mqls > 0 ? fmtPct((campaign.sqls / campaign.mqls) * 100) : '—'}
                      </td>
                      <td className="py-2 px-3 text-right text-gray-400">—</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-3 text-gray-600">Oportunidades</td>
                      <td className="py-2 px-3 text-right font-medium text-gray-800">{fmtN(campaign.opportunities)}</td>
                      <td className="py-2 px-3 text-right text-gray-400">—</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-3 text-gray-600">Reuniões</td>
                      <td className="py-2 px-3 text-right font-medium text-gray-800">{fmtN(campaign.meetings)}</td>
                      <td className="py-2 px-3 text-right text-gray-400">—</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-3 text-gray-600">Vendas</td>
                      <td className="py-2 px-3 text-right font-medium text-gray-800">{fmtN(campaign.won)}</td>
                      <td className="py-2 px-3 text-right text-gray-500">{ratio(campaign.spend, campaign.won)}</td>
                    </tr>
                    {campaign.meetings > 0 && (
                      <tr>
                        <td className="py-2 px-3 text-gray-600">RR→Ganho</td>
                        <td className="py-2 px-3 text-right font-medium text-gray-800">
                          {fmtPct((campaign.won / campaign.meetings) * 100)}
                        </td>
                        <td className="py-2 px-3 text-right text-gray-400">—</td>
                      </tr>
                    )}
                    {campaign.won > 0 && (
                      <>
                        <tr>
                          ...
                          <td className="py-2 px-3 text-right font-medium text-gray-800">{fmtBRL(campaign.mrr)}</td>
                          <td className="py-2 px-3 text-right text-gray-400">—</td>
                        </tr>
                        <tr>
                          <td className="py-2 px-3 text-gray-600">Ticket Médio</td>
                          <td className="py-2 px-3 text-right font-medium text-gray-800">
                            {campaign.mrr > 0 ? fmtBRL(campaign.mrr / campaign.won) : '—'}
                          </td>
                          <td className="py-2 px-3 text-right text-gray-400">—</td>
                        </tr>
