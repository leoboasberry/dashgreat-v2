...
export default function ExcludedUtmsFilter({ allUtms, excluded, onChange }: Props) {
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 z-[100] w-72 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
          <div className="px-3 py-2.5 border-b border-gray-100 flex items-center justify-between">
            <span className="text-xs font-medium text-gray-600">Excluir UTMs de todas as métricas</span>
            {count > 0 && (
              <button
                onClick={() => onChange([])}
          ...
          {allUtms.length === 0 ? (
            <p className="text-xs text-gray-400 px-3 py-3 text-center">Nenhuma UTM disponível</p>
          ) : (
